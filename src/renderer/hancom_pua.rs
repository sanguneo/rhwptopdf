...
mod tests {
    ...
    #[test]
    fn verified_table_stays_ascending_for_binary_search() {
        // 이진 탐색 전제(오름차순) 회귀 가드.
        let mut prev: u32 = 0;
        for cp in [
            0xF012B, 0xF02FB, 0xF02FC, 0xF031C, 0xF03A0, 0xF03C5, 0xF03EF, 0xF03F0, 0xF03F1,
            0xF03F2, 0xF03F3, 0xF03F4,
        ] {
            assert!(cp > prev, "PUA 표가 오름차순이 아님: {cp:#X} <= {prev:#X}");
            prev = cp;
        }
    }
}
