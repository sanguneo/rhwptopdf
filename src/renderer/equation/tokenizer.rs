...
impl Tokenizer {
    ...
    fn read_command(&mut self) -> Token {
        ...
        // HWP 수식 markup 에서 `root2`, `sqrt3` 같이 키워드가 숫자/식별자에 공백 없이
        // 붙어 들어오는 경우가 있다 (실 케이스: "3-11월_실전_통합_2022.hwp" 11번 문항).
        // 분리하지 않으면 `root2` 가 단일 식별자로 토큰화되어 parser 가 `\sqrt` 인식
        // 실패 → SVG 에 raw "root2" 텍스트가 그대로 그려진다.
        for kw in ["root", "sqrt", "ROOT", "SQRT", "Root", "Sqrt"] {
            if self.matches_at(kw) {
                let after = self.peek(kw.len());
                if matches!(after, Some(c) if c.is_ascii_alphanumeric()) {
                    self.pos += kw.len();
