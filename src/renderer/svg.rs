...
/// in-memory 폰트 맵으로 @font-face CSS 생성.
///
/// `fonts` 의 key 는 SvgRenderer 가 수집한 font-family 이름과 일치해야 한다.
/// Subset / Full 모드는 해당 폰트가 맵에 있을 때만 base64 임베드하며,
/// 그 외에는 `local()` 폴백.
pub fn generate_font_style_in_memory(
    ...
    let local_fallback = |font_name: &str, css: &mut String| {
    ...
    match renderer.font_embed_mode {
        FontEmbedMode::Style | FontEmbedMode::None => {
            for font_name in codepoints.keys() {
                local_fallback(font_name, &mut css);
            }
        }
        FontEmbedMode::Subset => {
            for (font_name, chars) in codepoints.iter() {
                if let Some(font_data) = fonts.get(font_name) {
                    ...
                    if let Ok(subset_data) = subsetter::subset(font_data, 0, &remapper) {
                        let b64 =
